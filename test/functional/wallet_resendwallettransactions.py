...
class ResendWalletTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Select outputs that can actually cover the transaction.  Auxpow
        # changes may cause a different order in listunspent, so the Bitcoin
        # logic of just picking the first outputs does not work reliably.
        parent_utxo, indep_utxo = [u for u in node.listunspent() if u["amount"] > 1][:2]
