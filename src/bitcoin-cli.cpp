    ...
    UniValue ProcessReply(const UniValue& reply) override
    {
        if (!reply["error"].isNull()) {
            if (reply["error"]["code"].getInt<int>() == RPC_METHOD_NOT_FOUND) {
                throw std::runtime_error("-addrinfo requires namecoind v26.0 or later which supports getaddrmaninfo RPC. Please upgrade your node or use namecoin-cli from the same version.");
            }
            return reply;
        }
